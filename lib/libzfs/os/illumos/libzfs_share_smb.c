...
/*
 * illumos-specific SMB sharing support for libzfs.
 *
 * SMB sharing on illumos is managed by the in-kernel CIFS server
 * (smbd/smbadm).  This is a stub implementation for now, similar
 * to FreeBSD.
 */
...
{
	if (shareopts != NULL &&
	    (strcmp(shareopts, "off") == 0 || strcmp(shareopts, "on") == 0))
		return (SA_OK);
