...
/*
 * procfs_list implementation for illumos using kstat adapter.
 *
 * On illumos we back the procfs_list with a kstat of type KSTAT_TYPE_RAW.
 * The show/show_header/clear callbacks match the Linux seq_file interface
 * used in the common OpenZFS code.
 */
...
/*
 * seq_printf — write a formatted string into a seq_file buffer.
 */
void
seq_printf(struct seq_file *f, const char *fmt, ...)
{
	...
	va_start(adx, fmt);
	n = vsnprintf(f->sf_buf + f->sf_off, f->sf_size - f->sf_off,
	    fmt, adx);
	va_end(adx);

	if (n > 0 && f->sf_off + n < f->sf_size)
		f->sf_off += n;
}

/*
 * seq_puts — write a string into a seq_file buffer.
 */
void
seq_puts(struct seq_file *f, const char *s)
{
	size_t len = strlen(s);

	if (f->sf_off + len < f->sf_size) {
		bcopy(s, f->sf_buf + f->sf_off, len);
		f->sf_off += len;
		f->sf_buf[f->sf_off] = '\0';
	}
}
...
{
	procfs_list_t *pl = ksp->ks_private;
	void *elt = pl->pl_next_data;
	pli_t *p = NULL;

	if (n == 0)
		pl->pl_next_data = list_head(&pl->pl_list);
	else if (elt)
		pl->pl_next_data = list_next(&pl->pl_list, elt);

	if (pl->pl_next_data) {
		p = kmem_alloc(sizeof (*p), KM_SLEEP);
		p->pli_pl = pl;
		p->pli_elt = pl->pl_next_data;
	}
...
{
	/* Nothing to do; cleanup happens in destroy. */
}
