    ...
    int flag, int *direntflags, pathname_t *realpnp)
{
	...
	*dlpp = NULL;
	 ...
	 * filesystem when the lookup request is not exact because
	 * normalization can fold case independent of normalizing code point
	 * sequences.
	 *
	...
	if (zfsvfs->z_norm != 0) {
		...
		/*
		 * Determine if the match needs to honor the case specified
		 * in lookup, and if so keep track of that so that during
	...
	if ((flag & ZSHARED) && ++dl->dl_sharecnt > 1 &&
	    dl->dl_namesize == 0) {
		/*
 ...
 * (force) umounted the file system.  Executed as an async taskq task.
 */
static void
...
zfs_rmnode(znode_t *zp)
{
	...
	if (ZTOV(zp)->v_type == VDIR && (zp->z_pflags & ZFS_XATTR)) {
		 ...
		 * XATTR directories because we need truncate and remove to
		 * be in the same tx, like in zfs_znode_delete(). Otherwise,
		 * if we crash here we'll end up with an inconsistent
		 * truncated zap object in the delete queue.  Note a
		 * truncated file is harmless since it only contains user
		 * data.
		 */
		error = dmu_free_long_range(os, zp->z_id, 0, DMU_OBJECT_END);
		if (error) {
			/*
			 * Not enough space or we were interrupted by
			 * unmount.  Leave the file in the unlinked set.
			 */
			zfs_znode_dmu_fini(zp);
			zfs_znode_free(zp);
			return;
	...
	if (error) {
		 ...
		 * which point we'll call zfs_unlinked_drain() to process
		 * it).
		 */
		dmu_tx_abort(tx);
		zfs_znode_dmu_fini(zp);
	...
	VERIFY3U(0, ==,
	    zap_remove_int(zfsvfs->z_os, zfsvfs->z_unlinkedobj, zp->z_id,
	    tx));
    ...
    int flag)
{
	...
	if (zp->z_zfsvfs->z_norm) {
		...
		error = zap_remove(zp->z_zfsvfs->z_os, dzp->z_id,
		    dl->dl_name, tx);
	}
 ...
 * held, but can only be considered a hint in the latter case.  Returns
 * true if only "." and ".." remain and there's no work in progress.
 */
boolean_t
 ...
 *
 *	OUT:	xvpp	- pointer to extended attribute vnode
 *
 *	RETURN:	0 on success
...
top:
	...
	error = zfs_make_xattrdir(zp, &va, &xzp, cr);
	zfs_dirent_unlock(dl);
