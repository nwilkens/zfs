 ...
 *
 * Ported from illumos-joyent zfs_znode.c to the OpenZFS
 * os/illumos platform layer.  On illumos, znodes embed vnodes
 * (vn_alloc/vn_free/vn_reinit).  The znode->vnode relationship
 * is the core of the illumos VFS integration.
 */
...
zfs_extend(znode_t *zp, uint64_t end)
{
	...
	VERIFY(0 == sa_update(zp->z_sa_hdl, SA_ZPL_SIZE(zfsvfs),
...
/*
 * Update the VFS layer to reflect znode changes. On illumos this is a no-op
 * since the vnode size is maintained separately via the VM system.
 */
void
zfs_znode_update_vfs(znode_t *zp)
{
	/* Nothing to do on illumos - vnode size is not cached in VFS layer */
}
    ...
    uint64_t buflen)
{
	...
	if ((err = sa_bulk_lookup(zp->z_sa_hdl, bulk, count)) != 0)
		return (err);
...
{
	/*
	 * On illumos, the RLIMIT_FSIZE check is done by the VFS layer
	 * (via the VOP_WRITE path). Return 0 unconditionally.
	 */
	return (0);
}
...
/*
 * The following functions (zfs_sa_setup, zfs_grab_sa_handle,
 * zfs_release_sa_handle, zfs_obj_to_pobj, zfs_obj_to_stats_impl,
 * zfs_obj_to_path_impl, zfs_obj_to_path, zfs_obj_to_stats) are
 * provided by the shared module/zfs/zfs_znode.c in OpenZFS.
