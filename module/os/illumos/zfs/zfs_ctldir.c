 ...
 * Currently, this is only the 'snapshot' and 'shares' directories,
 * but this may expand in the future.  The elements are built using the
 * GFS primitives, as the hierarchy does not actually exist on disk.
 *
 * For 'snapshot', we don't want to have all snapshots always mounted,
 * because this would take up a huge amount of space in /etc/mnttab.
 ...
 *                                         mounted fs
 *
 * The 'snapshot' node contains just enough information to lookup '..'
 * and act as a mountpoint for the snapshot.  Whenever we lookup a
 * specific snapshot, we perform an automount of the underlying
 * filesystem and return the corresponding vnode.
 *
 * All mounts are handled automatically by the kernel, but unmounts are
 * (currently) handled from user land.  The main reason is that there
 * is no reliable way to auto-unmount the filesystem when it's "no
 * longer in use".  When the user unmounts a filesystem, we call
 * zfsctl_unmount(), which unmounts any snapshots within the snapshot
 * directory.
 *
 * The '.zfs', '.zfs/snapshot', and all directories created under
 * '.zfs/snapshot' (ie: '.zfs/snapshot/<snapname>') are all GFS nodes
 * and share the same vfs_t as the head filesystem (what '.zfs' lives
 * under).
 *
 * File systems mounted ontop of the GFS nodes
 * '.zfs/snapshot/<snapname>' (ie: snapshots) are ZFS nodes and have
 * their own unique vfs_t.  However, vnodes within these mounted on
 * file systems have their v_vfsp fields set to the head filesystem
 * to make NFS happy (see zfsctl_snapdir_lookup()).  We VFS_HOLD the
 * head filesystem's vfs_t so that it cannot be freed until all
 * snapshots have been unmounted.
 */
...
/*
 * Forward declarations for illumos VFS stub functions defined in
 * the kernel's fs_subr module (not declared in headers available here).
 */
extern int fs_inval();
...
static vnode_t *zfsctl_snapshot_mknode(vnode_t *, uint64_t);
...
/*
 * Initialize the various GFS pieces we'll need to create and manipulate
 * .zfs directories.  This is called from the ZFS init routine, and
 * initializes the vnode ops vectors that we'll be using.
 */
void
zfsctl_init(void)
{
...
/*
 * Create the '.zfs' directory.  This directory is cached as part of the
 * VFS structure.  This results in a hold on the vfs_t.  The code in
 * zfs_umount() therefore checks against a vfs_count of 2 instead of 1.
 * This reference is removed when the ctldir is destroyed in the unmount.
 */
void
zfsctl_create(zfsvfs_t *zfsvfs)
{
	...
	/*
	 * We're only faking the fact that we have a root of a filesystem
	 * for the sake of the GFS interfaces.  Undo the flag manipulation
	 * it did for us.
	 */
 ...
 * unmounted.  There might still be more references if we were force
 * unmounted, but only new zfs_inactive() calls can occur and they
 * don't reference .zfs
 */
void
 ...
 * snapshot if it exists, creating the pseudo filesystem vnode as
 * necessary.  Perform a mount of the associated dataset on top of
 * the vnode.
 */
/* ARGSUSED */
static int
...
domount:
	...
	if (err == 0) {
		/*
		 * Return the mounted root rather than the covered mount
		 * point.  Takes the GFS vnode at .zfs/snapshot/<snapname>
		 * and returns the ZFS vnode mounted on top of the GFS node.
		 * This ZFS vnode is the root of the newly created vfsp.
		 */
		VFS_RELE(vfsp);
		err = traverse(vpp);
	}
...
zfsctl_lookup_objset(vfs_t *vfsp, uint64_t objsetid, zfsvfs_t **zfsvfsp)
{
	...
	if (sep != NULL) {
		...
		/*
		 * Return the mounted root rather than the covered mount
		 * point.  Takes the GFS vnode at
		 * .zfs/snapshot/<snapshot objsetid> and returns the ZFS
		 * vnode mounted on top of the GFS node.  This ZFS vnode
		 * is the root of the vfs for objset 'objsetid'.
		 */
		error = traverse(&vp);
		if (error == 0) {
			if (vp == sep->se_root)
...
int
zfsctl_make_fid(zfsvfs_t *zfsvfsp, uint64_t object, uint32_t gen,
    fid_t *fidp)
{
	zfid_short_t *zfid;
	int i;
...
int
zfsctl_rename_snapshot(const char *from, const char *to)
{
	/*
	 * Snapshot rename is handled internally by DSL;
	 * this entry point is called from the ioctl path.
	 */
	return (0);
}

int
zfsctl_destroy_snapshot(const char *snapname, int force)
{
	/*
	 * Snapshot destroy is handled internally by DSL;
	 * this entry point is called from the ioctl path.
	 * We just need to ensure it's unmounted first.
	 */
	(void) zfsctl_snapshot_unmount(snapname, force ? MS_FORCE : 0);
	return (0);
}
