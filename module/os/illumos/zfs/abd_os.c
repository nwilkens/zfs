 ...
 *
 * On illumos, scatter chunks are allocated from a kmem_cache of
 * PAGESIZE-sized buffers, identical to the FreeBSD approach.
 */
 ...
 */
static size_t zfs_abd_scatter_min_size;	/* set in abd_init() */
...
abd_update_scatter_stats(abd_t *abd, abd_stats_op_t op)
{
	...
	int waste = (n * PAGESIZE) - abd->abd_size;
	if (op == ABDSTAT_INCR) {
...
abd_verify_scatter(abd_t *abd)
{
	...
	ASSERT3U(ABD_SCATTER(abd).abd_offset, <, PAGESIZE);
	n = abd_scatter_chunkcnt(abd);
	for (i = 0; i < n; i++) {
...
abd_alloc_struct_impl(size_t size)
{
	 ...
	 * ABD struct as opposed to an ABD scatter with 0 chunks.
	 */
 ...
 * each chunk in the scatterlist will be set to the same zeroed area.
 */
static void
abd_alloc_zero_scatter(void)
{
	...
	/*
	 * Use a single zero-filled page for all chunks.
	 */
	abd_zero_buf = kmem_zalloc(PAGESIZE, KM_SLEEP);
	for (i = 0; i < n; i++) {
		ABD_SCATTER(abd_zero_scatter).abd_chunks[i] = abd_zero_buf;
	}
...
abd_free_zero_scatter(void)
{
	...
	kmem_free(abd_zero_buf, PAGESIZE);
	abd_zero_buf = NULL;
}
...
abd_init(void)
{
	zfs_abd_scatter_min_size = PAGESIZE + 1;
	abd_chunk_cache = kmem_cache_create("abd_chunk", PAGESIZE, 0,
...
{
	/*
	 * On illumos we don't use linear page ABDs (no vm_page_t mapping).
	 * This should not be called, but just in case, ASSERT.
	 */
	ASSERT(!"abd_free_linear_page called on illumos");
}
 ...
 * using a scatter/gather list we should switch to that.
 */
abd_t *
    ...
    size_t size)
{
	...
	if (abd == NULL)
		abd = abd_alloc_struct(chunkcnt * PAGESIZE);

	ABD_SCATTER(abd).abd_offset = new_offset % PAGESIZE;

	/* Copy the scatterlist starting at the correct offset */
	(void) memcpy(&ABD_SCATTER(abd).abd_chunks,
	    &ABD_SCATTER(sabd).abd_chunks[new_offset / PAGESIZE],
...
/*
 * Map the current chunk into aiter.
 */
void
abd_iter_map(struct abd_iter *aiter)
{
	...
	if (abd_is_linear(abd)) {
		...
		paddr = ABD_SCATTER(abd).abd_chunks[offset / PAGESIZE];
		offset %= PAGESIZE;
...
/*
 * Unmap the current chunk from aiter.
 */
void
...
/*
 * Return a borrowed raw buffer to an ABD.
 */
void
