 ...
 *
 * This file is adapted from the illumos-joyent vdev_disk.c to conform
 * to the OpenZFS vdev_ops_t interface.  It uses the Layered Driver
 * Interface (LDI) for all disk access.
 */
...
/*
 * The vdev_os_t type (our per-vdev state) is defined in
 * <sys/vdev_os.h> and stored in vdev_t.vdev_tsd.  It contains:
 *
...
vdev_disk_alloc(vdev_t *vd)
{
	...
	dvd = vd->vdev_tsd = kmem_zalloc(sizeof (vdev_os_t), KM_SLEEP);
	/*
...
vdev_disk_free(vdev_t *vd)
{
	...
	kmem_free(dvd, sizeof (vdev_os_t));
	vd->vdev_tsd = NULL;
}

/* ARGSUSED */
static int
vdev_disk_off_notify(ldi_handle_t lh, ldi_ev_cookie_t ecookie,
    ...
    int ldi_result, void *arg, void *ev_data)
{
	...
	/*
	 * If the offline state change was unsuccessful, request a probe
	 * to recheck the device state.
	 */
    ...
    int ldi_result, void *arg, void *ev_data)
{
	 ...
	 */
	(void) vdev_degrade(vd->vdev_spa, vd->vdev_guid, VDEV_AUX_NONE);
}
...
skip_open:
	...
	*logical_ashift = highbit64(MAX(blksz, SPA_MINBLOCKSIZE)) - 1;
	*physical_ashift = highbit64(MAX(pbsize, SPA_MINBLOCKSIZE)) - 1;
	...
	/* Secure TRIM is not currently supported on illumos. */
...
vdev_disk_io_start(zio_t *zio)
{
	...
	case ZIO_TYPE_FLUSH:
		...
		if (error == 0) {
			/*
			 * The ioctl will be done asynchronously,
